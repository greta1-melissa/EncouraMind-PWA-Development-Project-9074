  ...
  return ( 
    <>
      ...
      <motion.nav 
        ...
        className={`fixed left-0 top-0 bottom-0 w-64 z-50 ${isDark ? 'bg-gray-800' : 'bg-white'} shadow-xl lg:shadow-none lg:translate-x-0 lg:static lg:z-0`} 
      >
        ...
        <div className="p-4 space-y-2">
          {navItems.map(item=> ( 
            <Link 
              ...
              to={item.path} 
              onClick={onClose} 
              className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all group ${location.pathname===item.path ? 'bg-gradient-to-r from-primary-500 to-accent-600 text-white shadow-lg' : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'}`} 
            >
              <SafeIcon 
                icon={item.icon} 
                className={`w-5 h-5 transition-transform group-hover:scale-110 ${location.pathname===item.path ? 'text-white' : 'text-gray-500 dark:text-gray-400'}`} 
              />
