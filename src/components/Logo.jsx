...
const Logo = ({size = 'md', className = '', showText = false, spinning = false}) => {
  ...
  return (
    <div className={`relative overflow-hidden flex items-center justify-center ${showText ? 'flex-col' : ''} ${className}`}>
      <motion.div 
        className={`${sizeClass} rounded-full overflow-hidden flex items-center justify-center shadow-lg relative`}
      ...
      >
        ...
        {/* Fallback SVG with updated color scheme - hidden by default */}
      ...
      {/* Glow effect with updated colors - only when not spinning */}
      {!spinning && (
