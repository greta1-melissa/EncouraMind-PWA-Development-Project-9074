  ...
  return (
    <div className={`relative flex items-center justify-center ${showText ? 'flex-col' : ''} ${className}`}>
      ...
      >
        ...
        {/* Fallback SVG with updated calming colors - hidden by default */}
        <div className="hidden w-full h-full bg-gradient-to-br from-primary-500 via-secondary-500 to-accent-500 rounded-full flex items-center justify-center">
      ...
      {/* Glow effect with updated calming colors - only when not spinning */}
      {!spinning && (
        <div className="absolute inset-0 bg-gradient-to-r from-primary-400/20 to-secondary-400/20 rounded-full blur-xl opacity-70 -z-10"></div>
      )}
    </div>
  );
};
